...
% BAYES_LINEAR_FIT_ARD estimates w such that y = Xw, using Bayesian
% regularisation with automatic relevance determination.
...
% N(w | 0, (tau A)^-1) Gam(tau | a0, b0), with A being a diagonal matrix
% with the vector alpha = (alpha_1, ..., alpha_D)' along its diagonal, 
% and the hyper-priors p(alpha_i) = p(alpha | c0, d0). The prior parameters
% a0, b0, c0, d0 are set such that the prior is non-informative.
% The returned posterior (computed by variational Bayesian inference) is of
% the form N(w1 | w, tau^-1 V) Gam(tau | an, bn). Also, the mean vector E_a =
