function vb_logit_pred_incr_test()
%% unit tests for vb_logit_pred_incr(.)
...
fprintf('Testing vb_logit_pred_incr(.)\n');
fprintf('Size of return values                      ');
X = genX(N_small);
y = genY(X);
[w, V, invV] = vb_logit_fit(X, y);
py = vb_logit_pred_incr(X, w, V, invV);
...
y = genY(X);
[w, V, invV] = vb_logit_fit(X, y);
py1 = vb_logit_pred_incr(X, w, V, invV);
py2 = vb_logit_pred_incr(X, w, V, invV);
...
y = genY(X);
[w, V, invV] = vb_logit_fit(X, y);
ypred = 2 * (vb_logit_pred_incr(X, w, V, invV) > 0.5) - 1;
