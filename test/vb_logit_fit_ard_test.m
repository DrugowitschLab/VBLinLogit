function vb_logit_fit_ard_test()
%% unit tests for vb_logit_fit_ard(.)
...
fprintf('Testing vb_logit_fit_ard(.)\n');
...
elseif ~all(size(E_a) == [length(wgen) 1])
...
X = genX(N_small);
y = genY(X);
[w1, V1, invV1, logdetV1, E_a1, L1] = vb_logit_fit_ard(X, y);
[w2, V2, invV2, logdetV2, E_a2, L2] = vb_logit_fit_ard(X, y);
if norm(w1 - w2) > 1e-10
...
X = genX(N_small);
y = genY(X);
[w1, V1] = vb_logit_fit_ard(X, y);
[w2, V2] = vb_logit_fit_ard(X, y, a0);
[w3, V3] = vb_logit_fit_ard(X, y, a0, b0);
