...
% BAYES_LOGIT_POST returns a vector containing p(y=1 | x, X, Y) for x =
...
% start first ieration with xi = 0, lam_xi = 1/8
xi = zeros(N, 1);
lam_xi = lam(xi);
a_xi = 1 ./ (4 + xVx);
...
for i = 1:max_iter
    ...
    % apply Sherman Morrison formula to update w
    w_xi = Vwt - (a_xi * ones(1, Dx)) .* VxxVwt;
...
% posterior from variational bound
